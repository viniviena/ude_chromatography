...
function loss(θ)
    pred = predict(θ)
    #data = data_train
    sum(abs2, train_data[1] - pred[1]) + sum(abs2, train_data[2] - pred[2])
end

@time grad = Zygote.gradient(loss, θ)[1]
@time grad2 = FiniteDiff.finite_difference_gradient(loss, θ)
@time grad3 = Zygote.gradient(loss, θ)[1]
